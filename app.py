...
if page == "Historical explorer":
    ...
    st.subheader(f"{region}: heat-death % trend, 2016-2024")
    region_trend = panel[panel["region"] == region].sort_values("year").set_index("year")
    st.line_chart(region_trend[["attrib_deaths_heatwave_pct"]])

    st.subheader("Baseline vs. recommended capacity (this region-year)")
    compare_df = pd.DataFrame(
        {"Beds": [row["hospital_beds"], row["hospital_beds"] + (extra_beds or 0)]},
        index=["Baseline", "Recommended"],
    )
    st.bar_chart(compare_df)

    st.subheader(f"All regions compared, {year}")
    year_slice = panel[panel["year"] == year].set_index("region")[["attrib_deaths_heatwave_pct"]]
    st.bar_chart(year_slice.sort_values("attrib_deaths_heatwave_pct", ascending=False))
