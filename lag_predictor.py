...
if __name__ == "__main__":
    # Quick sanity check using real historical data: pick a known real heatwave
    # period and confirm the function behaves as expected.
    df = pd.read_csv("/home/claude/daily_dss/daily_merged_panel.csv", parse_dates=["Date"], low_memory=False)
    dep = "75"  # Paris
    sub = df[df["dep"] == dep].sort_values("Date").reset_index(drop=True)
